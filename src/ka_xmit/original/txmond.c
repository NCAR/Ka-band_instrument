...
#define TXMON_STAT_LEN 32 /* STX, 3 bytes (bit coded) + 12 bytes (3 four
...
static time_t time_last_print = 0;
...
static char hvps_voltage[8];
static char mag_current[8];
static char hvps_current[8];
static char tempC[8];
    ...
    if (retcode < 0) {
      ...
      if ((tx_stat(tx_status) == 0) && 
        ...
        if (difftime(fault_time, last_fault_time) > 10) {
           ...
           * the last fault */
	  fprintf(stderr, "Trying reset and operate\n");
          if (tx_reset() == -1)
            exit(EXIT_FAILURE);
          fault_reset = 1;
          if (tx_operate() == -1)
            exit(EXIT_FAILURE);
	  fprintf(stderr, "Reset and operate successful\n");
        } else {
          /* Only reset the transmitter
           * 3 times */
          if (fault_reset < 3) {
	    fprintf(stderr, "Trying reset and operate, try: %d\n", fault_reset);
            if (tx_reset() == -1)
              exit(EXIT_FAILURE);
            fault_reset++;
           if (tx_operate() == -1)
...
int tx_if_init()
{
  ...
  tcsetattr(tty_fd, TCSADRAIN, &newtio);
...
int tx_stat(char *stat)
{
  ...
  if (debug) {
    fprintf(stderr, "==>> Sending status command\n");
  }
  ...
  if (fd_read_select(tty_fd, 1000) != 1) {
    printf_log_mesg("Initial select failed - unable to read transmitter status (%m)");
    tx_if_flush();
    return -1;
  }
  ...
  for (i = 0; i < TXMON_STAT_LEN; i++) {
    ...
    if (fd_read_select(tty_fd, 50) != 1) {
      printf_log_mesg("Secondary select failed - unable to read transmitter status (%m)");
      tx_if_flush();
      return -1;
    }
    char cc;
    if ((ret = read(tty_fd, &cc, 1)) == -1) {
      printf_log_mesg("Read failed - unable to read transmitter status (%m)");
      tx_if_flush();
      return -1;
    }
    if (debug) {
  ...
  if (!etx_found) {
    printf_log_mesg("No ETX found - unable to read transmitter status (%m)");
    tx_if_flush();
    return -1;
  }
...
/*       printf_log_mesg("%02x", stat[i]); */
  ...
  if (debug) {
    printf_log_mesg("chksum read: %d", chksum_read);
    printf_log_mesg("chksum computed: %d", chksum);
  }
  
  if (chksum & 0x7f) {
    printf_log_mesg("Warning - checksum error, checksum: %d", chksum);
    tx_if_flush();
    // return -1;
  }
  
  memset(hvps_voltage, 0, 8);
  memset(mag_current, 0, 8);
  memset(hvps_current, 0, 8);
  memset(tempC, 0, 8);

  strncpy(hvps_voltage, stat + 4, 4);
  strncpy(mag_current, stat + 8, 4);
  strncpy(hvps_current, stat + 12, 4);
  strncpy(tempC, stat + 16, 3);

  if (debug) {
    fprintf(stderr, "HVPS voltage: %s\n", hvps_voltage);
    fprintf(stderr, "MAG current: %s\n", mag_current);
    fprintf(stderr, "HVPS current: %s\n", hvps_current);
    fprintf(stderr, "Temp C: %s\n", tempC);
  }

  time_t now = time(NULL);
  if (now - time_last_print >= 60) {
    printf_log_mesg("Checking status, HVPS voltage: %s, MAG current: %s, HVPS current: %s, temp: %s",
		    hvps_voltage, mag_current, hvps_current, tempC);
    time_last_print = now;
  }
...
int tx_stat_old(char *stat)
{
  ...
  if (debug) {
    ...
    for (j = 0; j < TXMON_STAT_LEN; j++)
      printf_log_mesg("%02x", stat[j]);
  }
